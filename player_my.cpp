...
int Alpha_Beta_Pruning(int A, int B, Point point, int depth, std::ofstream& fout){
    ...
    now.done = false;
    now.put_disc(point);
    if(depth == 0 || now.done) return now.getval();
    int tboard[8][8];
    ...
    if(play == BLACK){
        ...
        for(auto p : spots){
            ...
            value = max(value, Alpha_Beta_Pruning(A, B, p, depth - 1, fout));
            A = max(A, value);
            if(A >= B) break;
        }
        return value;
        ...
        for(auto p : spots){
            now.cur_player = play;
            for(int i = 0; i < 8; i ++) for(int j = 0; j < 8; j ++) now.board[i][j] = tboard[i][j];
            value = min(value, Alpha_Beta_Pruning(A, B, p, depth - 1, fout));
            B = min(B, value);
            if(A <= B) break;
        }
        return value;
...
void write_valid_spot(std::ofstream& fout) {
    ...
    now.cur_player = BLACK;
    for(auto pnt : next_valid_spots){
        for(int i = 0; i < 8; i ++) for(int j = 0; j < 8; j ++) now.board[i][j] = board[i][j];
        now.cur_player = BLACK;
        value = max(value, Alpha_Beta_Pruning(A, B, pnt, 10, fout));
        if(value > A) pos = idx;
        A = max(A, value);
        idx++;
    }
